...
def _looks_like_task_creation(call: ast.Call, taskflow_function_names: set[str]) -> bool:
    name = call_name(call)
    if not name:
        return False
...
class _StaticTaskLoopVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        name = call_name(node)
        if self.loop_counts and _looks_like_task_creation(node, self.taskflow_function_names):
            if all(count is not None for count in self.loop_counts):
                ...
                if estimated_count > STATIC_TASK_CREATION_LIMIT:
                    self.violations.append(
                        (node.lineno, node.col_offset, name, f"estimated loop-created tasks={estimated_count}")
                    )
            elif len(self.loop_counts) >= 2:
                self.violations.append(
                    (node.lineno, node.col_offset, name, "nested dynamic loop-created task count is unknown")
                )
        self.generic_visit(node)
...
def _find_excessive_static_task_creation(py_path):
    ...
    try:
        ...
        return []
    visitor = _StaticTaskLoopVisitor(_taskflow_function_names(tree), _module_iterable_counts(tree))
    visitor.visit(tree)
...
def test_static_task_creation_from_loops_is_limited():
    ...
    assert not offenders, (
        ...
        + "\n".join(
            f"  {fname}: " + ", ".join(f"line {ln}:{col} call={name} ({reason})" for ln, col, name, reason in hits)
