...
def _is_session_factory_call(node, aliases) -> bool:
    return isinstance(node, ast.Call) and _resolve_alias(call_name(node), aliases) in _SESSION_FACTORY_CALLS


def _metadata_session_names(tree, aliases):
    names = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign) and _is_session_factory_call(node.value, aliases):
            for target in node.targets:
                names.update(_target_names(target))

        if isinstance(node, ast.AnnAssign) and _is_session_factory_call(node.value, aliases):
            names.update(_target_names(node.target))

        if isinstance(node, ast.With):
            for item in node.items:
                if _is_session_factory_call(item.context_expr, aliases) and item.optional_vars:
        ...
        if isinstance(node, ast.ImportFrom) and node.module:
            for alias in node.names:
                ...
                if full_name in _DISALLOWED_IMPORTS:
                    violations.append((node.lineno, node.col_offset, f"import {full_name}"))

        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            for decorator in node.decorator_list:
                name = call_name(decorator) if isinstance(decorator, ast.Call) else call_name(ast.Call(func=decorator, args=[], keywords=[]))
                resolved = _resolve_alias(name, aliases)
                if resolved in _DISALLOWED_DECORATORS:
                    violations.append((decorator.lineno, decorator.col_offset, f"decorator {resolved}"))
