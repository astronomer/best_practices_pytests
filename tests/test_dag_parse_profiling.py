...
DEFAULT_STATEMENT_LIMIT_S = float(
    os.getenv("AIRFLOW_STATEMENT_TIME_LIMIT_S", "0.5")
)

# Overall file threshold — if the file imports fast, skip per-statement reporting
DEFAULT_FILE_LIMIT_S = float(
    os.getenv("AIRFLOW_DAG_IMPORT_TIME_LIMIT_S", "1.0")
)
...
def _profile_file(py_path):
    ...
    for node in tree.body:
        ...
        record_src = f"_timings.append(({lineno}, {end_lineno}, _time_mod.perf_counter() - _t0))"
